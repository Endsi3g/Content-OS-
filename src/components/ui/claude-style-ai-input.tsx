      ...
      <div className="flex items-start gap-3 size-[125px] overflow-hidden">
        ...
        {!isImage && (
          <div className="flex-1 min-w-0 overflow-hidden">
            ...
            <p
              className="max-w-[90%] text-xs font-medium text-zinc-100 truncate flex-1 z-10 relative"
              title={file.file.name}
            >
              {file.file.name}
            </p>
            <p className="text-[10px] text-zinc-500 mt-1 z-10 relative">
...
export const ClaudeChatInput: React.FC<ChatInputProps> = ({
  ...
  }, [message, files, pastedContent, disabled, isSending, onSendMessage, setMessage]);
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#30302E] border border-[var(--border)] rounded-xl shadow-lg items-end gap-2 min-h-[120px] flex flex-col transition-colors focus-within:border-gray-400">
        <textarea
          ref={textareaRef}
          value={message}
          ...
          className="flex-1 min-h-[80px] w-full p-4 focus-within:border-none focus:outline-none focus:border-none border-none outline-none focus-within:ring-0 focus-within:ring-offset-0 focus-within:outline-none max-h-[250px] resize-none border-0 bg-transparent text-white shadow-none focus-visible:ring-0 placeholder:text-zinc-500 text-sm sm:text-base custom-scrollbar"
          rows={1}
        />
        <div className="flex items-center gap-2 justify-between w-full px-3 pb-1.5 pt-1">
          <div className="flex items-center gap-2">
            <Button
              size="icon"
              variant="ghost"
              className="h-8 w-8 p-0 text-zinc-400 hover:text-white hover:bg-zinc-700 flex-shrink-0 rounded-full"
              onClick={() => fileInputRef.current?.click()}
              disabled={disabled || isSending || files.length >= maxFiles}
              title={
            ...
            {/* Keeping SlidersHorizontal disabled for now as per original code */}
          </div>
          <div className="flex items-center gap-2">
        ...
        {(files.length > 0 || pastedContent.length > 0) && (
          <div className="overflow-x-auto border-t-[1px] p-3 border-zinc-700 w-full bg-[#262624] hide-scroll-bar rounded-b-xl">
